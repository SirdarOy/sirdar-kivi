...
class Kivi_Admin {
  ...
  /**
 * Get the latest XML to fetch
 * KIVI offers listing of XML files, need to get the latest (=last) one
 */
public function getKiviXML(){
  $homepageurl = get_option('kivi-homepage-url');
  if(!$homepageurl) {
    $homepageurl = "http://kivi.etuovi.com/homepage/homepagecustomer_id_2143/";
  }
  $homepagecontent = file_get_contents($homepageurl);
  if( $homepagecontent !== FALSE ) {
      $dom=new DOMDocument;
    $dom->loadHTML( $homepagecontent );
    $col=$dom->getELementsByTagName('li');
    $lastIndex = (int)($col->length)-1;
    $lastfile = ($col[$lastIndex]->nodeValue);
    $lastfileurl = $homepageurl.$lastfile;
    $lastfileurl=preg_replace('/\s+/', '', $lastfileurl);
    return $lastfileurl;
  }
}

/**
 * Parsing that file to array of item objects
 * Returns array of objects containing the same properties as described in KIVI documentation
 */
public function fetchAndParseKiviXML($xmlurl){
  if($xmlurl){
    ...
    foreach($xml->item as $item){
      ...
      $itemObj->id = $id;
      $itemObj->images = [];
      $itemObj->realtyrealtyoptions = [];
      $itemObj->presentations = [];
      foreach($properties as $property){
        //Get names
        $name = (string)$property->attributes()->name;
        //Get images
        //Images are in one <property>
        if($name=="image"){
          //Which has multiple <property>s in it (for each image)
          foreach($property as $image){
              $imageObj = new stdClass();
              $imageAsArray = (array)$image;
              //And each image has multiple <property>s in it
              $singleimgproperties = $imageAsArray["property"];
              $pushthisimage = false;
            foreach($singleimgproperties as $imageproperty){
              $name = (string)$imageproperty->attributes()->name;
              $value = (string)$imageproperty->value;
                if($name=="image_url"){
                if (strpos($value, 'realty/www/') !== false) {
                    $pushthisimage = true;
                    $imageObj->image_url = $value;
                  }
                }
                else if($name=="image_realtyimagetype_id"){
                  $imageObj->image_realtyimagetype_id = $value;
                }
                else if($name=="image_iv_order"){
                  $imageObj->image_iv_order = $value;
                }
            }
            //And then push the object to the image array
            if($pushthisimage===true){
              array_push($itemObj->images, $imageObj);
            }
          }
        }
        //Get realtyoptions
        //Again in one <property>
        elseif($name=="realtyrealtyoption"){
          //Which has multiple <property>s in it (for each option)
          $allrealtyoptions = [];
          foreach($property as $option){
            $optionObj = new stdClass();
            $optionAsArray = (array)$option;
            $singleoptionproperties = $optionAsArray["property"];
            $pushthisoption = false;
            $group = null;
            $realvalue = null;
            foreach($singleoptionproperties as $optionproperty){
              $name = (string)$optionproperty->attributes()->name;
              $value = (string)$optionproperty->value;
              if($value){
                  if($name=="realtyoption_id"){
                    $realvalue = $value;
                  }
                  else if($name=="realtyoptiongroup_id"){
                    $group = $value;
                  }
              }
            }
            ...
            if($realvalue && $group){
              $itemObj->realtyrealtyoptions["$group"] = array("$realvalue");
            }
          }
        }
        //Get presentation
        elseif($name=="kivipresentation"){
          //Again in one <property> there is <property>ies for dates
          foreach($property as $option){
            $optionObj = new stdClass();
            $optionAsArray = (array)$option;
            //And each image has multiple <property>s in it
            $singlepresentation = [];
            $singlepresentationproperties = $optionAsArray["property"];
            foreach($singlepresentationproperties as $presentationproperty){
              $name = (string)$presentationproperty->attributes()->name;
              $value = (string)$presentationproperty->value;
              if($name && $value){
                $singlepresentation[$name] = $value;
              }
            }
            $itemObj->presentations[] = $singlepresentation;
          }
        }
        //Save if it has direct value
        elseif($property->value && $name){
  ...
  public function kivi_sync() {
    ...
    $items = $this->fetchAndParseKiviXML($this->getKiviXML());
